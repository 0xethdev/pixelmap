...
const PixelPortfolio = ({ filterActive, setFilterActive, setPixelData, toggleSetPixelData, tempPixelData, setTempPixelData }) => {
    ...
    const onColorSelect = (newInput) => {
        ...
        updatedColors[colorPickerIndex] = newInput;
                            ...
                            {userPixels.sort((a, b) => a.y - b.y || a.x - b.x).map((pixel, index) => (
                                <div key={index} className='flex justify-between items-center p-2 mb-2 border-b-2 border-darkgrey'>
                                    ...
                                    <div className='flex flex-1 items-center gap-1 w-[24px]'>
                                        <button onClick={() => openColorPicker(index)} className='text-xs text-black bg-lightgrey border-2 border-darkgrey hover:border-lightgrey py-1 px-2 w-full flex items-center justify-between'>
                                            <span>
                                                Pick Color
                                            </span>
                                            <svg className='ml-auto h-[16px] w-[16px]' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"> <path d="M9 2h2v2H9V2zm4 4V4h-2v2H9v2H7v2H5v2H3v2h2v2h2v2h2v2h2v2h2v-2h2v-2h2v-2h2v6h2V12h-2v-2h-2V8h-2V6h-2zm0 0v2h2v2h2v2h2v2H5v-2h2v-2h2V8h2V6h2z" fill='currentColor'/> </svg>
                                        </button>
                                    </div>
                                </div>
                            ))}
